...
import { FileSpreadsheet, Plus, Edit, Trash2, Image as ImageIcon, Loader2 } from 'lucide-react';
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 md:mb-8 gap-4">
        <div>
          <h1 className="text-2xl md:text-3xl font-black text-stone-900 tracking-tight">Inventario Base</h1>
          <p className="text-stone-500 mt-1 text-xs md:text-sm font-medium">Conectado a PostgreSQL</p>
        </div>
        <div className="flex space-x-2 md:space-x-3 w-full md:w-auto">
          ...
          <button onClick={openCreateModal} className="flex-1 md:flex-none bg-indigo-600 text-white justify-center px-4 md:px-6 py-2.5 md:py-3 rounded-xl md:rounded-2xl font-black flex items-center shadow-lg active:scale-[0.98] text-xs md:text-sm hover:bg-indigo-700">
            <Plus className="w-4 h-4 md:w-5 md:h-5 mr-1.5" /> Nuevo
          </button>
        </div>
      </div>
        ...
        <table className="w-full text-left border-collapse min-w-[500px]">
          <thead>
            <tr className="bg-stone-50 border-b border-stone-200 text-[10px] md:text-xs uppercase tracking-widest text-stone-500 font-black">
              ...
              <th className="p-3 md:p-5">Producto</th>
          ...
          <tbody className="text-xs md:text-sm font-medium">
            {filteredProducts.length === 0 ? (
              <tr><td colSpan={7} className="p-8 text-center text-stone-500">No hay productos.</td></tr>
            ) : (
                ...
                <tr key={product.id} className="border-b border-stone-100 hover:bg-stone-50/80">
                  ...
                  <td className="p-3 md:p-5 font-black text-indigo-600">
                    ...
                    {(product.promoPrice && product.promoPrice > 0 && product.promoPrice < product.price) && (
                      <div className="text-[10px] text-rose-500">Promo: ${product.promoPrice.toFixed(2)}</div>
                    )}
                  </td>
                  <td className="p-3 md:p-5 text-center">
                    <div className="flex items-center justify-center space-x-1 md:space-x-2">
                      <button onClick={() => openEditModal(product)} className="p-1.5 md:p-2 text-stone-400 hover:text-indigo-600 bg-stone-50 rounded-lg md:rounded-xl">
                        <Edit className="w-3 h-3 md:w-4 md:h-4" />
                      </button>
                      <button onClick={() => handleDeleteProduct(product.id)} className="p-1.5 md:p-2 text-stone-400 hover:text-rose-600 bg-stone-50 rounded-lg md:rounded-xl">
