...
import { Building2, Landmark, Save, Loader2, DollarSign, FileText, MapPin, Phone, ArrowLeft, User, Lock, Mail } from 'lucide-react';
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (userProfile.password.trim() !== '') {
        userPayload.password = userProfile.password;
      }
      const updatedUserRes = await axios.put(`${API_USER}/profile`, userPayload, { headers });
      
      // Actualizamos el localStorage para que el nombre cambie en toda la app
      ...
      // 3. Guardar Tasas (Si tienen valor)
      ...
      setUserProfile(prev => ({...prev, password: ''})); // Limpiamos el campo de contraseña
    } catch (error) {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label className="flex items-center gap-2 text-sm font-bold text-stone-600 mb-2">
                <User className="w-4 h-4" /> Nombre de Administrador
              </label>
              <input 
                type="text" 
                required
                ...
                placeholder="Ej: Angel Castro" 
                className="w-full bg-stone-50 border border-stone-200 rounded-2xl px-4 py-3 outline-none focus:ring-2 focus:ring-violet-500 transition-all font-medium"
              />
            </div>
            <div>
              <label className="flex items-center gap-2 text-sm font-bold text-stone-600 mb-2">
            ...
            <div className="md:col-span-2">
              <label className="flex items-center gap-2 text-sm font-bold text-stone-600 mb-2">
                <Lock className="w-4 h-4" /> Nueva Contraseña <span className="text-[10px] text-stone-400 font-normal">(Déjalo en blanco si no deseas cambiarla)</span>
              </label>
              <input 
                type="password" 
                ...
                placeholder="*********" 
                className="w-full md:w-1/2 bg-stone-50 border border-stone-200 rounded-2xl px-4 py-3 outline-none focus:ring-2 focus:ring-rose-500 transition-all font-medium"
              />
            </div>
          </div>
        </div>
